...
UNSUPPORTED = 6;//have't represented this key pattern yet
...
var RedisNamespaceClient = function(prefix, redis) {
    ...
    function generateMethod(method_name) {
        return function() {
            args = arguments;
            if(COMMANDS[method_name] == UNSUPPORTED) {
                throw('Unsupported Redis Command - ' + method_name);
            }
    ...
    for(var method_name in COMMANDS) {
        RedisNamespaceClient.prototype[method_name] = generateMethod(method_name);
    }
}
